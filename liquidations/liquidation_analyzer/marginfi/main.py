...
def run_full_pipeline(
    ...
    try:
        ...
        step2_output_file = Path(output_dir) / f"marginfi_liquidations_parsed_{timestamp}.json"
        ...
        step3_output_file = Path(output_dir) / f"marginfi_liquidation_analysis_{timestamp}.json"
...
def run_from_step(
    ...
    if from_step <= 2:
        ...
        print("\n【步骤 2】解析清算交易")
        step2_output = Path(output_dir) / f"marginfi_liquidations_parsed_{timestamp}.json"
    ...
    if from_step <= 3:
        ...
        print("\n【步骤 3】分析清算数据")
        step3_output = Path(output_dir) / f"marginfi_liquidation_analysis_{timestamp}.json"
...
def main():
    parser = argparse.ArgumentParser(
        description='Fast MarginFi - MarginFi 清算 MEV 分析工具',
