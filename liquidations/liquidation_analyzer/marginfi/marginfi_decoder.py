#!/usr/bin/env python3
"""
MarginFi Protocol 独立解析器
基于 IDL 完全解析 MarginFi 协议的所有指令

功能：
1. 解析所有 MarginFi 指令（借贷、清算、闪电贷等）
2. 解码指令参数（u64, u128, pubkey, bytes 等）
3. 映射账户名称
4. 支持批量交易解析
5. 提取清算/借贷关键信息

Program ID: 2jGhuVUuy3umdzByFx8sNWUAaf5vaeuDm78RDPEnhrMr
"""
...
# MarginFi 程序 ID
MARGINFI_PROGRAM_ID = "2jGhuVUuy3umdzByFx8sNWUAaf5vaeuDm78RDPEnhrMr"
MARGINFI_PROGRAM_ID_V2 = "MFv2hWf31Z9kbCa1snEPYctwafyhdvnV7FZnsebVacA"

# 所有 MarginFi 程序 ID
MARGINFI_PROGRAM_IDS = {MARGINFI_PROGRAM_ID, MARGINFI_PROGRAM_ID_V2}
...
class ParsedInstruction:
    ...
    raw_data_hex: str
    instruction_type: str  # liquidation, deleverage, borrow, repay, deposit, withdraw, flash_loan, init, other
...
class ParsedTransaction:
    ...
    marginfi_instructions: List[ParsedInstruction]
    is_liquidation: bool
    is_flash_loan: bool
    total_instructions: int
    marginfi_instruction_count: int
...
class MarginfiDecoder:
    ...
    def _build_instruction_map(self) -> None:
        ...
        MarginFi IDL 直接提供 discriminator 数组，不需要计算
        """
    ...
    def get_instruction_type(self, instruction_name: str) -> str:
        ...
        if instruction_name in LIQUIDATION_INSTRUCTIONS:
            return 'liquidation'
        elif instruction_name in DELEVERAGE_INSTRUCTIONS:
            return 'deleverage'
                ...
                for i, acc_info in enumerate(instruction_info['accounts']):
                    if i < len(accounts):
                        mapped_accounts.append({
                            ...
                            'address': accounts[i],
                            # MarginFi IDL 使用 writable/signer/optional
    ...
    def _parse_type(self, data: bytes, type_spec: Any) -> Tuple[Any, int]:
        ...
        if isinstance(type_spec, str):
            ...
            if 'array' in type_spec:
                ...
                # 自定义类型
                # MarginFi IDL 格式: {"defined": {"name": "TypeName"}}
    ...
    def decode_transaction(
        ...
        parsed_instructions = []
        marginfi_instructions = []
        unknown_instructions = []

        for instr in instructions:
            program_id = instr.get('programId', '')

            if program_id == MARGINFI_PROGRAM_ID:
                # 解析 MarginFi 指令
                data = instr.get('data', '')
                ...
                if parsed:
                    ...
                    marginfi_instructions.append(parsed)
                else:
                    # 尝试获取未知指令信息
        ...
        is_liquidation = (
            any(instr.instruction_type == 'liquidation' for instr in marginfi_instructions) or
            'LIQUIDAT' in helius_type.upper()
        )

        # 判断是否为闪电贷
        is_flash_loan = (
            any(instr.instruction_type == 'flash_loan' for instr in marginfi_instructions) or
        ...
        return ParsedTransaction(
            ...
            marginfi_instructions=[asdict(i) for i in marginfi_instructions],
            is_liquidation=is_liquidation,
            is_flash_loan=is_flash_loan,
            total_instructions=len(instructions),
            marginfi_instruction_count=len(
                marginfi_instructions) + len(unknown_instructions),
...
class MarginfiBatchProcessor:
    """MarginFi 批量交易处理器"""

    def __init__(self, decoder: MarginfiDecoder):
        self.decoder = decoder
        self.stats = {
            'total_transactions': 0,
            'marginfi_transactions': 0,
            'liquidations': 0,
            'flash_loans': 0,
            'borrows': 0,
            'repays': 0,
    ...
    def process_file(self, file_path: str) -> List[ParsedTransaction]:
        ...
        for tx in transactions:
            ...
            if parsed.marginfi_instruction_count > 0:
                self.stats['marginfi_transactions'] += 1
                ...
                # 统计解析和未知指令
                self.stats['parsed_instructions'] += parsed.marginfi_instruction_count - \
                    parsed.unknown_instruction_count
                self.stats['unknown_instructions'] += parsed.unknown_instruction_count

                # 统计指令类型
                for instr in parsed.marginfi_instructions:
    ...
    def print_stats(self) -> None:
        ...
        print(f"  MarginFi 交易: {self.stats['marginfi_transactions']:,}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='MarginFi 协议解析器')
    parser.add_argument(
        '--idl',
        default=str(Path(__file__).parent / 'idl (3).json'),
        help='IDL 文件路径'
    )
    parser.add_argument(
        '--mode',
    ...
    print("=" * 60)
    print("MarginFi 协议解析器")
    print("=" * 60)

    # 初始化解码器
    decoder = MarginfiDecoder(args.idl)
    ...
    if args.mode == 'info':
        ...
        processor = MarginfiBatchProcessor(decoder)
