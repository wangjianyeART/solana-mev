#!/usr/bin/env python3
"""
步骤 2: 批量解析 MarginFi 清算交易

功能：
- 批量解析步骤1获取的清算交易
- 使用 IDL 解析 MarginFi 指令
- 提取余额变化和清算详情

使用方法：
    python step2_batch_parse.py <input_json>
    python step2_batch_parse.py ./data/marginfi_liquidations_raw_20260130.json
"""
...
from marginfi_decoder import MarginfiDecoder, MARGINFI_PROGRAM_ID, MARGINFI_PROGRAM_IDS
...
KNOWN_TOKENS = {
    ...
    "3NZ9JMVBmGAqocybic2c7LQCJScmgsAZ6vQqTDzcqmJh": {
        "symbol": "WBTC", "decimals": 8, "name": "Wrapped BTC"
    },
    "HzwqbKZw8HxMN6bF2yFZNrht3c2iXXzpKcFu7uBEDKtr": {
        "symbol": "EURC", "decimals": 6, "name": "Euro Coin"
    },
}
...
class BatchLiquidationParser:
    ...
    def __init__(self, idl_path: Optional[str] = None):
        if idl_path is None:
            idl_path = str(Path(__file__).parent / "idl (3).json")
        self.decoder = MarginfiDecoder(idl_path)
    ...
    def parse_raw_transaction(self, tx_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for instr_type, parent_idx, instr in all_instructions:
            ...
            if program_id in MARGINFI_PROGRAM_IDS:
                result['is_marginfi_tx'] = True
        ...
        result['marginfi_instructions'] = marginfi_instructions
    ...
    def _extract_liquidation_details(
        ...
        marginfi_instructions: List[Dict]
    ) -> Dict[str, Any]:
        """提取清算详细信息"""
        details = {
            'liquidator': '',
        ...
        for instr in marginfi_instructions:
            if instr.get('name') == 'lending_account_liquidate':
                accounts = instr.get('accounts', [])
                args = instr.get('args', {})

                # 从参数中获取 asset_amount
                if 'asset_amount' in args:
                    details['asset_amount'] = args['asset_amount']
                ...
                for acc in accounts:
                    ...
                    # 清算人：signer 或名称包含 liquidator
                    if acc.get('is_signer') and not details['liquidator']:
                        details['liquidator'] = address
                    elif 'liquidator' in name and 'marginfi_account' in name:
                        # liquidator_marginfi_account 是清算人的账户
                        if not details['liquidator']:
                            details['liquidator'] = address

                    # 被清算人
                    if 'liquidatee' in name:
                        if 'marginfi_account' in name:
                            details['liquidatee'] = address

        token_changes = parsed_result.get('token_balance_changes', [])

        # 如果没有找到清算人，尝试从 token 变化中推断
        # 清算人通常是同时有正负变化的 owner（支付债务，获得抵押品）
        if not details['liquidator']:
            owner_changes = {}
            ...
            for owner, changes in owner_changes.items():
                if changes['positive'] and changes['negative']:
                    details['liquidator'] = owner
                    break

        # 分析所有 owner 的 token 变化
        owner_changes = {}
        for change in token_changes:
            owner = change.get('owner', '')
            if owner:
        ...
        # 找真正的清算人钱包（同时有正负变化，支付稳定币，获得非稳定币）
        liquidator_wallet = None
        for owner, changes in owner_changes.items():
            if changes['positive'] and changes['negative']:
                # 检查是否支付稳定币并获得非稳定币
                has_stable_payment = any(
                    c['symbol'] in ['USDC', 'USDT'] for c in changes['negative']
                )
                has_collateral_receive = any(
                    c['symbol'] not in ['USDC', 'USDT'] for c in changes['positive']
                )
        ...
        if liquidator_wallet:
            details['liquidator_wallet'] = liquidator_wallet

        # 找清算人支付的债务代币（负变化，优先稳定币）
        if liquidator_wallet and liquidator_wallet in owner_changes:
            for change in owner_changes[liquidator_wallet]['negative']:
                symbol = change.get('symbol', '')
                if symbol in ['USDC', 'USDT']:
                    details['debt_token'] = symbol
                    details['debt_amount'] = abs(change['change'])
                    break
                elif not details['debt_token']:
                    details['debt_token'] = symbol
                    details['debt_amount'] = abs(change['change'])

            # 找清算人收到的抵押品（正变化，非稳定币）
            for change in owner_changes[liquidator_wallet]['positive']:
                symbol = change.get('symbol', '')
                if symbol not in ['USDC', 'USDT']:
                    details['collateral_token'] = symbol
...
def batch_parse(
    ...
    if output_file is None:
        ...
        output_file = input_path.parent / \
            f"marginfi_liquidations_parsed_{timestamp}.json"
