#!/usr/bin/env python3
"""
Jupiter Lend (Vaults) Protocol 独立解析器
基于 IDL 完全解析 Jupiter Vaults 协议的所有指令

功能：
1. 解析所有 Jupiter Vaults 指令（清算、操作、再平衡等）
...
相关 Jupiter Lend 程序:
- Vaults (本解析器): jupr81YtYssSyPt8jbnGuiWon5f6x9TcDEFxYe3Bdzi
...
# Jupiter Lend 程序 ID
JUPITER_VAULTS_PROGRAM_ID = "jupr81YtYssSyPt8jbnGuiWon5f6x9TcDEFxYe3Bdzi"
JUPITER_LIQUIDITY_PROGRAM_ID = "jupeiUmn818Jg1ekPURTpr4mFo29p46vygyykFJ3wZC"
JUPITER_LENDING_PROGRAM_ID = "jup3YeL8QhtSx1e253b2FDvsMNC87fDrgQZivbrndc9"
JUPITER_ORACLE_PROGRAM_ID = "jupnw4B6Eqs7ft6rxpzYLJZYSnrpRgPcr589n5Kv4oc"
JUPITER_FLASHLOAN_PROGRAM_ID = "jupgfSgfuAXv4B6R2Uxu85Z1qdzgju79s6MfZekN6XS"
...
INIT_INSTRUCTIONS = {
    ...
    'init_position',
    'init_tick',
    'init_tick_has_debt_array',
    'init_tick_id_liquidation',
    'init_vault_admin',
    'init_vault_config',
    'init_vault_state',
}
...
class ParsedInstruction:
    ...
    raw_data_hex: str
    instruction_type: str  # liquidation, operate, rebalance, init, other
...
class ParsedTransaction:
    ...
    jupiter_instructions: List[ParsedInstruction]
    is_liquidation: bool
    total_instructions: int
    jupiter_instruction_count: int
...
class JupiterDecoder:
    """Jupiter Vaults 协议解码器"""
    ...
    def load_idl(self, idl_path: str) -> None:
        ...
        metadata = self.idl.get('metadata', {})
        print(f"✓ 已加载 IDL: {metadata.get('name')} v{metadata.get('version')}")
    ...
    def _build_instruction_map(self) -> None:
        """
        构建指令 discriminator 映射
        Jupiter IDL 直接提供 discriminator 数组，不需要计算
        """
        for instruction in self.idl.get('instructions', []):
            ...
            # 获取 discriminator（直接从 IDL 读取）
            disc_array = instruction.get('discriminator', [])
            discriminator = bytes(disc_array)
            ...
            self.instruction_map[discriminator] = instruction_info
            self.instruction_map_hex[discriminator.hex()] = instruction_info
    ...
    def get_instruction_type(self, instruction_name: str) -> str:
        ...
        if instruction_name in LIQUIDATION_INSTRUCTIONS:
            return 'liquidation'
        elif instruction_name in OPERATE_INSTRUCTIONS:
            return 'operate'
        elif instruction_name in REBALANCE_INSTRUCTIONS:
            return 'rebalance'
        elif instruction_name in INIT_INSTRUCTIONS:
            return 'init'
        else:
            return 'other'
                ...
                for i, acc_info in enumerate(instruction_info['accounts']):
                    if i < len(accounts):
                        mapped_accounts.append({
                            ...
                            'address': accounts[i],
                            # Jupiter IDL 使用 writable/signer/optional
                            'is_mutable': acc_info.get('writable', False),
                            'is_signer': acc_info.get('signer', False),
                            'is_optional': acc_info.get('optional', False),
                        })
    ...
    def _parse_type(self, data: bytes, type_spec: Any) -> Tuple[Any, int]:
        ...
        if isinstance(type_spec, str):
            ...
            if 'array' in type_spec:
                ...
                # 自定义类型
                # Jupiter IDL 格式: {"defined": {"name": "TypeName"}}
                defined = type_spec['defined']
                if isinstance(defined, dict):
                    type_name = defined.get('name', '')
                else:
                    type_name = defined
    ...
    def _parse_primitive(self, data: bytes, type_name: str) -> Tuple[Any, int]:
        ...
        elif type_name in ('publicKey', 'pubkey'):
            if len(data) < 32:
                raise ValueError("数据不足")
    ...
    def decode_transaction(
        ...
        for instr in instructions:
            ...
            if program_id == JUPITER_VAULTS_PROGRAM_ID:
                ...
                if parsed:
                    ...
                    jupiter_instructions.append(parsed)
                else:
                    # 尝试获取未知指令信息
        ...
        # 判断是否为清算交易
        is_liquidation = (
            any(instr.instruction_type == 'liquidation' for instr in jupiter_instructions) or
        ...
        return ParsedTransaction(
            ...
            jupiter_instructions=[asdict(i) for i in jupiter_instructions],
            is_liquidation=is_liquidation,
            total_instructions=len(instructions),
            jupiter_instruction_count=len(
                jupiter_instructions) + len(unknown_instructions),
...
class JupiterBatchProcessor:
    """Jupiter Vaults 批量交易处理器"""

    def __init__(self, decoder: JupiterDecoder):
        self.decoder = decoder
        self.stats = {
            'total_transactions': 0,
            'jupiter_transactions': 0,
            'liquidations': 0,
            'operates': 0,
            'rebalances': 0,
            'instruction_counts': {},
    ...
    def process_file(self, file_path: str) -> List[ParsedTransaction]:
        ...
        for tx in transactions:
            ...
            if parsed.jupiter_instruction_count > 0:
                self.stats['jupiter_transactions'] += 1

                if parsed.is_liquidation:
                    self.stats['liquidations'] += 1

                # 统计解析和未知指令
                self.stats['parsed_instructions'] += parsed.jupiter_instruction_count - \
                    parsed.unknown_instruction_count
                self.stats['unknown_instructions'] += parsed.unknown_instruction_count

                # 统计指令类型
                for instr in parsed.jupiter_instructions:
                    ...
                    if instr_type == 'operate':
                        self.stats['operates'] += 1
                    elif instr_type == 'rebalance':
    ...
    def print_stats(self) -> None:
        """打印统计信息"""
        print("\n" + "=" * 60)
        print("Jupiter Vaults 交易统计")
        print("=" * 60)

        print(f"\n总体统计:")
        print(f"  总交易数: {self.stats['total_transactions']:,}")
        print(f"  Jupiter 交易: {self.stats['jupiter_transactions']:,}")
        print(f"  清算交易: {self.stats['liquidations']:,}")

        print(f"\n操作统计:")
        print(f"  Operate 操作: {self.stats['operates']:,}")
        print(f"  Rebalance 操作: {self.stats['rebalances']:,}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Jupiter Vaults 协议解析器')
    parser.add_argument(
        '--idl',
        default=str(Path(__file__).parent / 'vaults.json'),
        help='IDL 文件路径'
    )
    parser.add_argument(
        '--mode',
    ...
    print("=" * 60)
    print("Jupiter Vaults 协议解析器")
    print("=" * 60)

    # 初始化解码器
    decoder = JupiterDecoder(args.idl)
    ...
    if args.mode == 'info':
        ...
        processor = JupiterBatchProcessor(decoder)
