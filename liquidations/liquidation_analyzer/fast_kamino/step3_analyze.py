...
class LiquidationAnalysis:
    ...
    # 成本（直接使用 signer 的 SOL 余额变化，不再单独看 fee）
    signer_sol_cost_sol: float   # signer SOL 余额减少量 (成本)
    signer_sol_cost_usd: float
...
def analyze_single_liquidation(tx: Dict[str, Any], sol_price: float = 250.0) -> LiquidationAnalysis:
    ...
    # 3. 成本：直接使用 signer（清算人）的 SOL 余额变化
    sol_balance_changes = tx.get("sol_balance_changes", [])
    signer_sol_cost_sol = 0.0
    for ch in sol_balance_changes:
        if ch.get("account") == liquidator and ch.get("change", 0) < 0:
            signer_sol_cost_sol += abs(ch["change"])
    if signer_sol_cost_sol == 0:
        # 若无 signer SOL 变化则回退到 fee
        signer_sol_cost_sol = tx.get("fee", 0) / 1e9
    ...
    signer_sol_cost_usd = signer_sol_cost_sol * sol_price_used
    ...
    net_profit_usd = gross_profit_usd - signer_sol_cost_usd
    ...
    return LiquidationAnalysis(
        ...
        signer_sol_cost_sol=signer_sol_cost_sol,
        signer_sol_cost_usd=signer_sol_cost_usd,
...
def analyze_liquidations(input_file: str, output_file: Optional[str] = None) -> List[LiquidationAnalysis]:
    ...
    for i, tx in enumerate(transactions):
        ...
        if analysis.uses_flash_loan:
            flash_loan_count += 1
        total_signer_sol_cost += analysis.signer_sol_cost_sol
    ...
    total_signer_sol_cost_usd = sum(r.signer_sol_cost_usd for r in results)

    print(f"\n成本统计 (Signer SOL 余额变化):")
    print(
        f"  总 Signer SOL 成本: {total_signer_sol_cost:.6f} SOL (${total_signer_sol_cost_usd:.4f})")
    if results:
        print(
            f"  平均 Signer SOL 成本: {total_signer_sol_cost/len(results):.6f} SOL")
    ...
    for token, count in sorted(debt_tokens.items(), key=lambda x: -x[1]):
        print(
            f"  {token}: {count} ({count/len(results)*100:.1f}%)" if results else f"  {token}: {count}")
    ...
    for i, r in enumerate(results[:10]):
        ...
        print(f"  成本 (Signer SOL 余额变化):")
        source_map = {"oracle": "预言机",
                      "historical": "历史价格", "api_current": "当前API"}
        price_source = source_map.get(r.sol_price_source, r.sol_price_source)
        print(
            f"    Signer SOL 成本: {r.signer_sol_cost_sol:.6f} SOL (${r.signer_sol_cost_usd:.4f}) [SOL=${r.sol_price_used:.2f} from {price_source}]")
        if r.uses_flash_loan:
    ...
    output_data = {
        "metadata": {
            ...
            "total_signer_sol_cost_sol": total_signer_sol_cost,
            "total_signer_sol_cost_usd": total_signer_sol_cost_usd,
