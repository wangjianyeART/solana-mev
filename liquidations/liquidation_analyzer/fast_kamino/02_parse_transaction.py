#!/usr/bin/env python3
"""
Step 2: Parse Kamino Liquidation Transactions
...
class LiquidationData:
    ...
    collateral_amount: float    # Received collateral amount
    ...
    obligation_owner: str       # Liquidatee (obligation owner)
...
def get_token_info(mint: str) -> Dict[str, Any]:
    ...
    return {
        "symbol": mint[:8] + "...",
        "decimals": 9,  # Default assumption: 9 decimals
        "name": "Unknown Token"
    }
...
class LiquidationParser:
    ...
    def __init__(self, idl_path: Optional[str] = None):
        ...
        Args:
            idl_path: Path to Kamino IDL file
        """
        if idl_path is None:
    ...
    def parse_transaction(self, tx_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            tx_data: Transaction data obtained from step 1
        ...
        # If it's a liquidation transaction, extract detailed info
    ...
    def _parse_token_balance_changes(
        ...
        for key in all_keys:
            ...
            if abs(change) > 1e-10:  # Ignore negligible changes
    ...
    def _parse_logs(self, logs: List[str]) -> Dict[str, Any]:
        ...
        for log in logs:
            ...
            # Liquidation-related logs - search for numeric patterns
    ...
    def _extract_liquidation_details(
        ...
        details = {
            ...
            'received_collateral': 0.0,      # Collateral actually received by the liquidator
            'collateral_amount': 0.0,        # Collateral used for calculation (= total collateral)
            'collateral_decimals': 9,
            'protocol_fee': 0.0,
            'protocol_fee_deducted': False,  # Whether the protocol fee has been deducted from received_collateral
        ...
        # First find the liquidator (the one paying USDC/USDT)
        ...
        # Find the liquidatee (person whose SOL-type tokens decreased, and is not the liquidator)
        for change in token_changes:
        ...
        # Find protocol fee (small SOL amount received by the liquidatee)
        ...
        # Verify whether the protocol fee has been deducted from received_collateral
        # If received_collateral + protocol_fee ~= total_collateral, then it was deducted
...
def parse_transaction(
    ...
    if verbose:
        ...
        if result['liquidation_details']:
            ...
            print(
                f"    Debt repaid: {details['debt_amount']:.6f} {details['debt_token']}")
            print(
...
def main():
    ...
    try:
        ...
        print(f"\n{'=' * 70}")
        print("Step 2 complete! You can now run step 3 to calculate profits")
