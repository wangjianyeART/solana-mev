...
使用方法：
    ...
    python step2_batch_parse.py ./data/liquidations_raw_20260130.json
"""
        ...
        for change in token_changes:
            if change['change'] < -0.01 and change['symbol'] in ['SOL', 'mSOL', 'stSOL', 'JitoSOL', 'bSOL']:
                if change['owner'] != details['liquidator']:
                    ...
                    details['collateral_amount'] = abs(change['change'])
...
def batch_parse(
    ...
    if output_file is None:
        ...
        output_file = input_path.parent / f"liquidations_parsed_{timestamp}.json"
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\n示例:")
        print("  python step2_batch_parse.py ./data/liquidations_raw_20260130.json")
        sys.exit(1)
