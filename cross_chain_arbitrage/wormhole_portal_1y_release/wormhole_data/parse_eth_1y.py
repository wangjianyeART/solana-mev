#!/usr/bin/env python3
"""
只解析 recent_1y/eth_raw/ 的 ETH raw,输出 eth_parsed.json。
(SOL 侧暂不解析,后续用 Wormhole API 反查 sol_sig,只解析需要的子集。)

用法: python wormhole_data/parse_eth_1y.py
"""
...
if ERC20_DECIMALS_FILE.exists():
    ...
    print(f"已加载 ERC20 decimals: {len(_dec_data)} 个 (TOKEN_CACHE 总计 {len(TOKEN_CACHE)})")
...
def main():
    ...
    dup_count = 0
    null_parse = 0
    for i, bf in enumerate(eth_batches, 1):
        ...
        for entry in batch:
            ...
            if parsed:
                ...
                null_parse += 1
        if i % 50 == 0:
            print(f"  {i}/{len(eth_batches)} batches done | parsed={len(eth_results)} dup={dup_count} null={null_parse} ({time.time()-t_start:.0f}s)")
    ...
    for r in eth_results:
        ...
        if r["src_chain"] == "Solana" or r["dst_chain"] == "Solana":
            sol_related += 1
    ...
    eth_output = {
        "meta": {
            ...
            "null_parse_skipped": null_parse,
            "sol_related": sol_related,
            "directions": eth_dirs,
            "routes": dict(sorted(eth_routes.items(), key=lambda x: -x[1])[:40]),
    ...
    print(f"\n  写出 eth_parsed.json ({len(eth_results)} 条)...")
    with open(ETH_OUT, "w", encoding="utf-8") as f:
        json.dump(eth_output, f, indent=2, ensure_ascii=False)

    print(f"\n  ETH: {len(eth_results)} 笔  errors={eth_errors}  dup={dup_count}  null={null_parse}")
    print(f"  方向: {eth_dirs}")
    print(f"  SOL-related (src 或 dst = Solana): {sol_related}")
    print(f"  Top 10 routes: {dict(list(sorted(eth_routes.items(), key=lambda x: -x[1]))[:10])}")
    print(f"  保存: {ETH_OUT}")
