...
\paragraph{符号约定。}
全章使用如下符号：$\Cexec^{\text{in}}$ 表示入场 swap 的 USD 等值
本金（execution capital, 即 entry notional）；
$f_{\text{total}}$ 为两链上 gas 与桥接费合计的总手续费；
$\mathit{gross}$ 为入场--出场两次 swap 合并计算的毛利润（未扣手续费）；
$\Pnet = \mathit{gross} - f_{\text{total}}$ 为扣除手续费后的净 PnL；
资本回报率定义为
\begin{equation}
\mathrm{ROI} \;=\; \Pnet \,\big/\, \Cexec^{\text{in}},
\label{eq:roi-def}
\end{equation}
文中一律以百分数呈现。
...
$+1.39\%$、$78.0\%$ 样本为正；SOL$\to$ETH 方向的 CDF 左尾略薄，
ETH$\to$SOL 略厚，但两方向中位 ROI 差 $<\!1$ 百分点，方向本身的
解释力有限。表~\ref{tab:roi-quantiles} 给出可靠候选的 ROI 完整
分位数结构；重尾特征十分明显：p1 仅 $-49\%$ 而 p99 高达 $+152\%$，
$\text{std}/\text{median}$ 超过 $48$ 倍，均值 $+7.83\%$ 被少数
极端正样本显著上拉，偏离中位数 $+1.39\%$ 达 $5.6$ 倍。

\begin{table}[h]
  \centering
  \small
  \caption{可靠候选的 ROI 分位数（$n=29{,}340$，单位 \%）}
  \label{tab:roi-quantiles}
  \begin{tabular}{lr|lr|lr}
    \toprule
    \textbf{统计量} & \textbf{值} & \textbf{分位} & \textbf{值} & \textbf{分位} & \textbf{值} \\
    \midrule
    count & $29{,}340$ & p1   & $-49.65$ & p75 & $+3.77$ \\
    mean  & $+7.83$    & p5   & $-4.15$  & p90 & $+10.66$ \\
    std   & $67.07$    & p10  & $-1.51$  & p95 & $+27.79$ \\
    min   & $-125.28$  & p25  & $+0.20$  & p99 & $+152.40$ \\
    max   & $+5{,}204.09$ & p50 & $+1.39$ & --- & --- \\
    \bottomrule
  \end{tabular}
\end{table}

ROI 最小值 $-125.28\%$（亏损超过 $100\%$）并非计算错误：依
式~\eqref{eq:roi-def}，当 $f_{\text{total}}>\Cexec^{\text{in}}$ 时
分子 $\Pnet$ 的负值量级可超过分母 $\Cexec^{\text{in}}$，
使 ROI 在数学上穿过 $-100\%$；最极端的 LOVE 样本入场本金
$\$1.00$、总手续费 $\$1.13$，实属真实的经济亏损而非测量误差。

\paragraph{资本与 PnL 几近无关（面板 c/f）。} 在对数横轴上，入场
本金跨越七个数量级。对 $n=29{,}128$ 的可靠候选以
\begin{equation}
  \Pnet = \alpha + \beta\cdot\log_{10}(\Cexec^{\text{in}}) + \varepsilon
  \label{eq:ols}
\end{equation}
拟合 OLS（$\alpha$ 为截距、$\beta$ 为斜率、$\varepsilon$ 为残差），
得到 $\hat\beta=-1{,}085$~\$/decade（即本金每增加一个 $10$ 倍量级，
$\Pnet$ 的期望下降约 $\$1{,}085$）、$R^2=0.0027$。也就是说，
\emph{本金仅解释净 PnL 方差的 $0.27\%$}。面板 (f) 的 binned median
曲线在 $10^2$--$10^4$ USD 区间近似平坦并略微大于零，两端显著下沉，
呈倒 U 形。这一反直觉结果否定了"跨链套利是资本可线性放大之生意"
这一朴素假设。

\paragraph{费用是一阶约束（面板 d/e）。} 费率
$\mathit{fee}/|\mathit{gross}|$（以 $\mathit{fee}=f_{\text{total}}$
为总手续费、$\mathit{gross}$ 为两次 swap 的毛利）的分布范围极宽，
中位 $0.053$，但有 $2.8\%$ 的候选出现 $\mathit{fee}>|\mathit{gross}|$
的严重侵蚀。面板 (e) 所有
...
ETH$\to$SOL（红）中位数 $\sim\!1{,}011$ 秒（$\sim\!17$ 分钟），两者
...
（$n=14{,}599$，中位 $+1.39\%$）与 $60$--$300$ 秒档
（$n=2{,}639$，中位 $+1.04\%$）均优于 $1$--$24$h 档
（中位 $+0.13\%$），$>\!24$h 档更跌至 $-1.35\%$；
$300$ 秒--$1$h 档样本量最大（$n=14{,}449$，中位 $+1.50\%$）
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    入场本金 & $R^2=0.003$，几无解释力 & 图~\ref{fig:economics}(f) \\
...
预测 $\mathbf{1}\{\Pnet>0\}$ 的盈亏方向（$\mathbf{1}\{\cdot\}$ 为
指示函数，条件为真取 $1$、为假取 $0$）。
...
\label{eq:reg-target}
\end{equation}
其中 $\operatorname{clip}(z,a,b)=\max\!\bigl(a,\min(b,z)\bigr)$ 为区间
截断；损失为半平方误差
...
(ii) $[-\$100,+\$500]$ 覆盖 $95.8\%$ 的候选样本（仅 $2.5\%$
下截、$1.7\%$ 上截），绝大多数样本保留原始值，仅尾部被 winsorize；
(iii) 上下限有意不对称：$\Pnet$ 的中位数 $+\$3.14$、$p_{75}\!\approx\!+\$10$，
...
模型输出概率 $\hat p_i=\sigma\!\bigl(F_M(x_i)\bigr)$，其中
$\sigma(z)=1/(1+e^{-z})$ 为 logistic sigmoid。下单决策由阈值
$\tau\in(0,1)$ 得出：$\hat y^{\text{cls}}_i = \mathbf{1}\{\hat p_i>\tau\}$。

\subsection{评估协议}

训练集限定为 \texttt{pnl\_complete} 且 17 特征无缺失的
$n=15{,}497$ 条。采用 $K=5$ 折随机 KFold
...
\label{eq:pr}
\end{equation}
其中 $TP,\,FP,\,FN$ 分别为给定阈值 $\tau$ 下的真阳、假阳、假阴
样本数（真阳 $=$ 模型判盈且实际盈利；假阳 $=$ 模型判盈但实际亏损；
假阴 $=$ 模型判亏但实际盈利）。
分类的朴素基线为"全部判为盈利"，准确率即数据正例率
$\bar y^{\text{cls}}=\tfrac{1}{n}\sum_{i=1}^{n} y^{\text{cls}}_i=73.85\%$。

\subsection{交叉验证结果}

表~\ref{tab:model} 汇总 5 折 CV 表现。回归
$R^2_{\text{CV}}=0.799\pm0.027$——相比式~\eqref{eq:ols} 的
单变量 $R^2=0.003$ 提升逾两个数量级，证明 \emph{$\Pnet$ 在本章
17 特征下是可预测的，但要求多变量非线性模型}。Median AE $=\$5.13$
意味着至少一半样本被预测到 $\pm\$5$ 以内；MAE $=\$17.14$ 与中位
误差的 $3.3$ 倍差距揭示——误差分布自身重尾，少数极端样本仍被
低估（这也印证了式~\eqref{eq:reg-target} 裁剪的必要性）。

分类 AUC $=0.866\pm0.005$，显著高于随机基线 $0.5$，但远未达到
"完美可分" $1.0$——反映跨链套利存在固有随机性（对手盘行为、
Guardian 签名抖动），本章特征集无法穷尽。阈值 $\tau=0.5$ 下
Accuracy $=82.3\%$，较 $\bar y^{\text{cls}}=73.85\%$ 的朴素基线
净提升 $+8.5$ pp。将阈值提高到 $\tau=0.8$，Precision 跃至
$93.5\%$，Recall 退至 $66.2\%$——意味着只在模型置信度 $>0.8$
时下单，下单中 $\sim\!94\%$ 实际盈利，代价是漏掉 $34\%$ 的机会。
这一 Precision--Recall 权衡直接支撑把模型作为策略侧的高置信度
过滤器。Log-Loss $=0.386$ 也优于基线二项分布的 $0.576$。

\begin{table}[h]
  \centering
  \small
  \caption{模型性能（5 折 CV, $n=15{,}497$）}
  \label{tab:model}
  \begin{tabular}{ll}
    \toprule
    \textbf{指标}                    & \textbf{值} \\
    \midrule
    回归 $R^2_{\text{CV}}$          & $0.7987 \pm 0.0269$ \\
    回归 MAE                         & \$17.14 \\
    回归 Median AE                   & \$5.13 \\
    分类 AUC$_{\text{CV}}$          & $0.8658 \pm 0.0052$ \\
    分类 Accuracy @$\tau=0.5$       & $82.3\%$（基线 $73.9\%$）\\
    分类 Precision @$\tau=0.8$      & $93.5\%$（Recall $66.2\%$）\\
    分类 Log-Loss                   & $0.3864$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\$5，被过滤的样本从 $12$ 条增至约 $116$ 条（$\sim0.4\%$），
reliable 子集的 ROI 均值从 $+7.83\%$ 下降到约 $+6.88\%$，
中位数几乎不变（$+1.39\%\to+1.38\%$）。两种阈值下
