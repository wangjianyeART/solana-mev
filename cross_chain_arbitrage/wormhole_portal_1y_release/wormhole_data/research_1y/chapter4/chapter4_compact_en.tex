...
(i)~\emph{On-chain verifiability.} Every Portal message is
cryptographically signed by the Guardian cluster and committed
on-chain as a Verifiable Action Approval (VAA), so that a Solana
signature and an Ethereum transaction hash can be paired 1:1 from
cryptographic evidence alone, without any off-chain ledger;
...
(iv)~\emph{Widest token coverage.} For the long-tail memecoins that
dominate observed cross-chain arbitrage, the token set supported
by Wormhole (via its WTT and NTT standards) is a strict superset of
the other four bridges, so ``Wormhole-only'' sampling introduces no
material selection bias for the phenomenon we study.

\subsection{Data Collection Pipeline}
\label{sec:pipeline}

Figure~\ref{fig:pipeline} shows our five-stage collection and matching
pipeline. The official Wormhole \texttt{/operations} API indexes by
VAA but \emph{only retains approximately $7$ days of history}, which
is insufficient for our year-long analysis. We therefore \emph{invert
the pipeline} and start from the on-chain Portal contracts on both
legs:
...
to retrieve the corresponding VAA (this path is not bounded by the
$7$-day window of the \texttt{/operations} API);
...
\begin{figure*}[t]
  ...
  \caption{Temporal structure: latency CDF by direction (a),
  atomicity matrix (b), swap--bridge gap (c), ROI by latency
  tier (d).}
  \label{fig:timing}
\end{figure*}
...
left-skewed, with median $+1.41\%$ and $75.7\%$ of samples positive;
...
$+151\%$; the ratio $\text{std}/|\text{median}|$ exceeds $47$; the
mean $+7.85\%$ is $5.5\times$ the median $+1.43\%$, dragged by a small
number of extreme positive samples.

\begin{table}[h]
  \centering
  \small
  \caption{ROI quantiles on reliable candidates ($n=28{,}788$, units \%)}
...
\begin{equation}
  ...
  \label{eq:ols}
\end{equation}
on the $n=28{,}586$ reliable candidates yields
$\hat\beta=-999$~\$/decade with $R^2=0.0022$.
In words, \emph{entry capital explains only $0.22\%$ of the variance
...
$\mathit{fee}/|\mathit{gross}|=0.054$, but $2.7\%$ of candidates
...
cross-chain candidates from four angles:
(a) bridge-latency CDF by direction---SOL$\to$ETH (blue) and
ETH$\to$SOL (red) are nearly coincident, both with median
$\sim\!800$~s ($\sim\!13$~min, matching two Ethereum epochs of
finality); the inter-direction gap is $<\!5\%$;
(b) atomicity matrix---both-atomic ($89.3\%$) and both-non-atomic
($6.7\%$) dominate, while asymmetric one-side-atomic cases are
small ($3.9\%+0.1\%$); fully-atomic attempts amount to $29$
candidates ($0.1\%$);
(c) swap-to-bridge time gap---the entry-side gap has a median
$<\!12$~s, the exit-side is longer ($\sim\!24$~s), with a right tail
extending into the hour range (a small number of candidates hold the
bridged position before exit);
(d) ROI boxplot by latency tier---medians drop monotonically from
the $<\!60$~s tier through $60$--$300$~s, $300$~s--$13$~min,
$13$~min--$24$~h; the $>\!24$~h tier has the smallest $n$ and the
widest dispersion. Panel~(d) is the direct evidence for the
``bridge latency negative-significant'' row of
Table~\ref{tab:univar}.
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    Bridge latency   & Negative, significant     & Fig.~\ref{fig:timing}(d) + model \\
    Liquidity depth  & Non-linear                & Liquidity-tier strata \\
    Fees             & Negative (identity)       & Fig.~\ref{fig:economics}(d/e) \\
    Entry capital    & $R^2\!=\!0.002$, near-null & Fig.~\ref{fig:economics}(f) \\
    Direction        & Gap $<\!1$\,pp             & Fig.~\ref{fig:timing}(a) \\
    Hour of day      & Approximately uniform     & (omitted) \\
    Entry/exit atomicity & No inter-tier ROI gap & Fig.~\ref{fig:timing}(b) \\
    Swap split count & No significant effect     & Fig.~\ref{fig:timing}(c) \\
    \bottomrule
  \end{tabular}
\end{table}
...
The target is a symmetrically USD-clipped $\Pnet$:
\begin{equation}
...
$L_{\text{reg}}(y,\hat y)=\tfrac12(y-\hat y)^2$. The clip bounds come
from the empirical $(p_1, p_{99})$ of ROI: samples beyond this range
load the $R^2$ asymmetrically and over-fit the tails.
...
\texttt{pnl\_complete} and no missing features, giving $n=15{,}259$.
...
reaches $R^2_{\text{CV}}=0.793\pm 0.032$---more than two orders of
magnitude above Eq.~\eqref{eq:ols}'s univariate $R^2=0.002$, proving
that \emph{$\Pnet$ is predictable given the $17$-feature set, but
only under a multivariate non-linear model}. Median AE $=\$5.04$
implies at least half of the samples are predicted within $\pm\$5$;
the gap between MAE $=\$16.96$ and Median AE (a factor of $3.4$)
...
The classifier's AUC is $0.862\pm 0.006$: substantially above
...
set cannot exhaust. At $\tau=0.5$, accuracy is $82.5\%$, $+7.8$~pp
over the naive $\bar y^{\text{cls}}=74.71\%$ baseline. Raising
$\tau$ to $0.8$ trades recall for precision: precision rises to
$93.4\%$ while recall falls to $66.8\%$---only trading when the
model is $>\!0.8$ confident yields a $\sim\!94\%$ realised-win rate
at the cost of missing $33\%$ of the opportunities. This
precision--recall schedule directly motivates using the classifier
as a high-confidence strategy-side filter. Log-loss $=0.384$ is also
below the binomial baseline of $0.566$.

\begin{table}[h]
  \centering
  \small
  \caption{Model performance (5-fold CV, $n=15{,}259$)}
  \label{tab:model}
  \begin{tabular}{ll}
    \toprule
    \textbf{Metric} & \textbf{Value} \\
    \midrule
    Regressor $R^2_{\text{CV}}$              & $0.7934 \pm 0.0318$ \\
    Regressor MAE                            & \$16.96 \\
    Regressor Median AE                      & \$5.04 \\
    Classifier AUC$_{\text{CV}}$             & $0.8620 \pm 0.0057$ \\
...
AUC only certifies the \emph{ranking} of probabilities; for the
$\hat p_i$ to be usable as a literal empirical probability (as in the
$\tau=0.8$ strategy above), the \emph{numerical values} must also be
trustworthy. We partition the pooled out-of-fold $\hat p$ into ten
equal-frequency buckets ($n_d=1{,}526$ each) and compare the bucket
mean $\bar{\hat p}_d$ with the observed positive rate
$\bar y_d=\tfrac{1}{n_d}\sum_{i\in d} y^{\text{cls}}_i$; see
Table~\ref{tab:calibration}. All absolute gaps
$|\bar{\hat p}_d-\bar y_d|<5$~pp; the largest is $-4.7$~pp in D2,
while the remaining nine bins deviate by $\leq 4$~pp. For the
high-confidence half of the distribution (D5--D10) the actual
positive rate exceeds the predicted probability by $2$--$3$~pp---the
model is \emph{slightly pessimistic} where it matters most for a
conservative trading rule.

\begin{table}[h]
  \centering
  \small
  \caption{Decile calibration of the classifier's 5-fold CV predicted
  probabilities (per-bin $n=1{,}526$)}
  \label{tab:calibration}
  \begin{tabular}{lrrr}
    \toprule
    \textbf{Bin} & \textbf{Pred.\ mean} & \textbf{Actual rate} & \textbf{Gap} \\
    \midrule
...
threshold to \$5 increases the set of excluded rows from $11$ to
$\approx 120$ ($\sim 0.4\%$), lowers the ROI mean on the reliable
subset from $+7.85\%$ to about $+4.2\%$, while leaving the median
essentially unchanged ($+1.43\%\!\to\!+1.45\%$). The GBM results of
...
out of scope.
Section~\ref{sec:bridges} argued that Wormhole has the broadest
memecoin coverage and the largest candidate volume among the five,
and that its ``messaging layer $+$ token wrapping'' structure permits
exact PnL reconstruction. That said, Portal's end-to-end latency
is a median of $\sim\!13$~s (Solana finality) plus $\sim\!13$~min
(two Ethereum epochs)---numbers specific to Wormhole. On LayerZero or
intent-based Mayan/Relay, the latency mechanism, tail shape, and the
arbitrage window differ substantially, so the chapter's model
coefficients and $R^2_{\text{CV}}$ should not be extrapolated to
other bridges; a rigorous cross-bridge comparison is future work.
...
On one year of Wormhole Portal data we construct $30{,}729$
cross-chain arbitrage candidates and, on the $28{,}574$ reliable
subset, provide descriptive and multivariate evidence in parallel.
The descriptive layer (\S\ref{sec:what-matters}) shows that both the
\emph{direction} and \emph{magnitude} of cross-chain profit are
\emph{not} determined by the micro-structural variables commonly
assumed to matter in the literature---entry capital, trade
direction, hour of day, or swap atomicity---while entry spread and
bridge latency are the two exogenous variables significantly
correlated with $\Pnet$ (Table~\ref{tab:univar}). The modelling
layer (\S\ref{sec:model}) achieves 5-fold cross-validated regression
$R^2_{\text{CV}}=0.79$, classification $\mathrm{AUC}_{\text{CV}}=0.86$,
and Median AE of \$5, showing that $\Pnet$ is \emph{predictable}
under the $17$-feature set; all ten decile-bucket calibration gaps
are $<\!5$~pp, letting the predicted probability feed directly into
strategy-side threshold decisions---the $\tau=0.8$ operating point
yields a $93.4\%$-precision high-confidence filter. Two follow-ups
suggest themselves: (a)~the upstream determinants of latency
heterogeneity (Guardian signing cadence, asymmetric chain finality);
and (b)~structural modelling of liquidity impact within the arbitrage
window, which is essential to characterise the true edge in
