#!/usr/bin/env python3
"""
Wormhole PORTAL_TOKEN_BRIDGE 1年 SOL↔ETH

采集过去365天所有包含 PORTAL_TOKEN_BRIDGE 的跨链记录。
这是真正的 lock & mint 跨链，token被锁在源链，目标链mint wrapped版本。

用法:
    python wormhole_data/fetch_wormhole_portal_1y.py
"""
...
OUTPUT = Path(__file__).parent / "wormhole_portal_1y.json"
...
def has_portal(op):
    content = op.get("content", {})
    std = content.get("standarizedProperties", {})
    return "PORTAL_TOKEN_BRIDGE" in std.get("appIds", [])
...
def save_json(records, scanned, tag=""):
    by_dir = {}
    by_token = {}
    total_usd = 0
    for r in records:
...
def main():
    ...
    print("=" * 65)
    print("  Wormhole Portal Token Bridge 1年采集")
    print("=" * 65)
    ...
    for win_start, win_end in windows:
        for direction in DIRECTIONS:
            ...
            print(f"  扫描{page_scanned} Portal+{kept} 累计{len(records):,} ({elapsed:.0f}s)")

            if len(records) - last_save >= SAVE_EVERY:
                save_json(records, scanned, "  (进行中)")
                last_save = len(records)

    elapsed = time.time() - t0
    print(f"\n完成: {elapsed:.0f}s, 扫描{scanned:,}, Portal={len(records):,}")
    save_json(records, scanned, "  [最终]")
