#!/usr/bin/env python3
"""
步骤 1 (快速版): 并发获取 Jupiter Lend 清算交易，严格遵守速率限制

功能：
1. 获取 Jupiter Vaults 交易签名
...
# Jupiter Vaults 清算指令 discriminator (直接从 IDL 获取)
# liquidate: [223, 179, 226, 125, 48, 46, 39, 74]
LIQUIDATION_DISCRIMINATORS: Set[str] = {
    bytes([223, 179, 226, 125, 48, 46, 39, 74]).hex(),  # liquidate
}
...
def fetch_liquidations_fast(limit: int = 5000, workers: int = 5, output_dir: str = "./data"):
    ...
    print("=" * 70)
    print(f"Jupiter Lend - 步骤 1: 并发获取清算交易")
    print("=" * 70)
    print(f"\n目标: 扫描 {limit} 条签名，只保存清算交易")
    print(f"程序 ID: {JUPITER_VAULTS_PROGRAM_ID}")
    ...
    while len(all_signatures) < limit:
        ...
        sigs = client.get_signatures_for_address(
            JUPITER_VAULTS_PROGRAM_ID, limit=batch_limit, before=before
        )
    ...
    output_file = output_path / f"jupiter_liquidations_raw_{timestamp}.json"

    save_data = {
        'metadata': {
            'protocol': 'Jupiter Lend',
            'program_id': JUPITER_VAULTS_PROGRAM_ID,
            'timestamp': timestamp,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='快速获取 Jupiter Lend 清算交易')
