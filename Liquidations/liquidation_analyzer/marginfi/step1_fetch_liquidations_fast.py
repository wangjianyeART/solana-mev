#!/usr/bin/env python3
"""
步骤 1 (快速版): 并发获取 MarginFi 清算交易，严格遵守速率限制
...
# MarginFi 清算指令 discriminators (直接从 IDL 获取)
# lending_account_liquidate: [214, 169, 151, 213, 251, 167, 86, 219]
# start_liquidation: [244, 93, 90, 214, 192, 166, 191, 21]
# end_liquidation: [110, 11, 244, 54, 229, 181, 22, 184]
LIQUIDATION_DISCRIMINATORS: Set[str] = {
    bytes([214, 169, 151, 213, 251, 167, 86, 219]).hex(),  # lending_account_liquidate
    bytes([244, 93, 90, 214, 192, 166, 191, 21]).hex(),    # start_liquidation
    bytes([110, 11, 244, 54, 229, 181, 22, 184]).hex(),    # end_liquidation
}
...
def fetch_liquidations_fast(limit: int = 5000, workers: int = 5, output_dir: str = "./data"):
    ...
    print("=" * 70)
    print(f"MarginFi - 步骤 1: 并发获取清算交易")
    print("=" * 70)
    print(f"\n目标: 扫描 {limit} 条签名，只保存清算交易")
    print(f"程序 ID: {MARGINFI_PROGRAM_ID}")
    ...
    while len(all_signatures) < limit:
        ...
        sigs = client.get_signatures_for_address(
            MARGINFI_PROGRAM_ID, limit=batch_limit, before=before
        )
    ...
    output_file = output_path / f"marginfi_liquidations_raw_{timestamp}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='快速获取 MarginFi 清算交易')
