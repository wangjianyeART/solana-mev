...
def detect_usdc_arbitrage(file_path):
    if not os.path.exists(file_path):
        print(f"错误: 找不到文件 {file_path}")
        return

    print(f"正在分析 USDC 套利交易: {file_path} ...")
    ...
    for slot_id, transactions in data.items():
        ...
        for tx in transactions:
            ...
            for balance in tx.get('post_token_balances', []):
                ...
                # 逻辑核心：区分 Signer 和 池子
                if owner == signer:
                    signer_usdc_profit += diff
                elif diff > 0:
                    # 记录非 Signer 账户（池子）收到的 USDC 增加量
                    pool_increases.append(diff)

            # --- 核心判定逻辑 ---
            # 1. 必须有池子变动，且 Signer 必须有正利润
            if not pool_increases or signer_usdc_profit <= 1e-6:
                continue
            ...
            if max_pool_inc > signer_usdc_profit * 1:
                leverage = signer_usdc_profit / max_pool_inc
                found_in_slot += 1
                ...
                print(f"套利者    : {signer}")
                print(f"套利净利  : {signer_usdc_profit:.4f} USDC")
                print(f"池子流水  : {max_pool_inc:.4f} USDC")
                print(f"利润率  : {leverage:.5f}%")
                print("💰"*20)

        if found_in_slot == 0:
            print(f"Slot {slot_id}: 未发现 USDC 套利交易。")


if __name__ == "__main__":
    file_name = "mev_full_analysis_252308623_252308633.json"
    detect_usdc_arbitrage(file_name)
